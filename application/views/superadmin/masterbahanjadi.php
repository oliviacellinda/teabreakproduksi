                                    ...
                                    <div class="row">
                                        <div class="col-md-3">
                                            <div class="form-group">
                                                <label for="kode" class=" form-control-label">Kode Bahan Jadi</label>
                                        ...
                                        <div class="col-md-3">
                                            <div class="form-group">
                                                <label for="harga" class=" form-control-label">Harga Jual Bahan Jadi</label>
                                                <input type="text" id="harga" placeholder="Masukkan Harga Jual" class="form-control numeric">
                                            </div>
                                        </div>
                                    </div>
                                    <div class="row">
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <table id="tabelData" class="table table-striped table-bordered" style="width: 100%;" width="100%">
                                    <thead>
                                        <tr>
                                            <td>Kode Bahan Jadi</td>
                                            <td>Nama Bahan Jadi</td>
                                            <td>Harga Jual</td>
                                            <td>Edit</td>
                                            <td>Delete</td>
                                        </tr>
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col-md-6 col-sm-12">
                            <div class="form-group">
                                <label for="editKode" class=" form-control-label">Kode Bahan Jadi</label>
                        ...
                        <div class="col-md-6 col-sm-12">
                            <div class="form-group">
                                <label for="editHarga" class=" form-control-label">Harga Jual Bahan Jadi</label>
                                <input type="text" id="editHarga" placeholder="Masukkan Harga Jual" class="form-control numeric">
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script type="text/javascript">
        ...
        function tambahBahanJadi() {
            ...
            if(kode.length > 0 && nama.length > 0 && harga.length > 0) {
                jQuery.ajax({
                    type    : 'post',
                    url     : '<?=base_url('superadmin/tambahBahanJadi');?>',
                    data    : {
                        kode    : kode,
                        nama    : nama,
                        harga   : harga,
        ...
        function editBahanJadi(kode) {
            jQuery.ajax({
                type    : 'post',
                url     : '<?=base_url('superadmin/ambilBahanJadi');?>',
                dataType: 'json',
                data    : { kode : kode },
                success : function(data) {
                    jQuery('#editKode').val(data.kode_bahan_jadi);
                    jQuery('#kodeLama').val(data.kode_bahan_jadi);
                    jQuery('#editNama').val(data.nama_bahan_jadi);
                    jQuery('#editHarga').val(data.harga_jual);
        ...
        function simpanEdit() {
            ...
            if(kode.length > 0 && nama.length > 0 && harga.length > 0) {
                jQuery.ajax({
                    type    : 'post',
                    url     : '<?=base_url('superadmin/editBahanJadi');?>',
                    data    : {
                        kode    : kode,
                        kodeLama: kodeLama,
                        nama    : nama,
        ...
        function hapusBahanJadi(kode) {
            if( confirm('Apakah Anda yakin ingin menghapus data ' + kode + '?') ) {
                jQuery.ajax({
                    type    : 'post',
                    url     : '<?=base_url('superadmin/hapusBahanJadi');?>',
                    data    : { kode : kode },
                    success : function() {
                        reloadTable();
                    },
                ...
                ajax : {
                    ...
                    dataSrc : function(datatable) {
                        ...
                        for(var i=0; i<datatable.data.length; i++) {
                            returnData.push({
                                'kode_bahan_jadi'   : datatable.data[i].kode_bahan_jadi,
                                'nama_bahan_jadi'   : datatable.data[i].nama_bahan_jadi,
                                'harga_jual'        : datatable.data[i].harga_jual,
                                'edit'              : '<button onclick=\"editBahanJadi(\''+datatable.data[i].kode_bahan_jadi+'\')\" class="btn btn-warning" style="color:white;">Edit</button>',
                                'hapus'             : '<button onclick=\"hapusBahanJadi(\''+datatable.data[i].kode_bahan_jadi+'\')\" class="btn btn-danger" style="color:white;">Hapus</button>',
                            });
                        }
                        return returnData;
                    },
